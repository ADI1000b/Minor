...
visualizeBtn.addEventListener('click',()=>{
    ...
    switch(algorithm){
        case 'backtracking':
            return backtracking(grid,speed);
        
        case 'bfs':
            return bfs(grid,speed);
        
        case 'algorithm-x':
            return algorithmX(grid,speed);
        default:
            return algorithmX(grid,speed);
    }
...
const backtracking = (grid,speed,comingFromGenerator=false,row=0,col=0,counter=null, animationList=null) =>{
    ...
    for(let possibleNum=1;possibleNum<=9;possibleNum++){
            ...
            if(isCellValid(nextRow,nextCol,grid)){
               ...
               if(backtracking(grid,speed,comingFromGenerator , nextRow,nextCol,counter,animationList)) return true;
...
const bfs = (grid,speed,row=0,col=0,counter=null, animationList=null) =>{
    ...
    for(let choice=0;choice<nextChoices.length;choice++){
            ...
            // here we are sure that choices are valid choices.
                ...
                animationList.push([nextRow, nextCol,possibleNum,'correct'])
               if(bfs(grid,speed,nextRow,nextCol,counter,animationList)) return true;
...
const animate = async (animationList, speed)=>{
